...
> 上下文是 LLM 的工作记忆。每次工具调用都在消耗它。当记忆耗尽，智能就崩塌了。

你的主 agent 在"思考"还是在"记流水账"？一次重构任务，十几轮 `read`、`edit`、`bash` 过后，上下文里堆满了工具调用痕迹和代码片段，最初的任务描述被挤到上下文的中段——恰好是模型注意力最薄弱的区域。

这不是假设，是被反复验证的规律：

- **"Lost in the Middle"**：LLM 对上下文开头和结尾的注意力远高于中间部分（Stanford, TACL 2024）。工具调用痕迹堆积越多，关键任务信息就越靠近"中间盲区"——模型不再"看见"它。
- **断崖式衰减**：超过有效上下文长度后，模型的理解能力下降超过 **30%**。
- **不可逆的压缩**：用摘要或上下文压缩来腾空间，被裁掉的细节就永久丢失了——你没法从概括里恢复精度。

上下文管理不是优化手段，是 LLM agent 能否正常工作的生存条件。

**subagent-isolation** 是 [Pi Agent](https://github.com/earendil-works/pi-coding-agent) 的扩展。它把具体执行委派给独立的子 agent，每个子 agent 在独立 `pi` 进程中运行，拥有干净的上下文和精确隔离的能力。主 agent 的上下文里只保留"做什么"和"结果如何"——中间的工具调用痕迹全部留在子 agent 的进程里。
...
| 所有 skill 和工具堆在主 agent 里，容易互相干扰 | 每个子 agent 只加载任务需要的 skill，能力精确隔离 |
...
- **递归边界**：子 agent 可配置为禁止继续向下委派，确保任务范围始终可控。
