...
> A `pi` extension that delegates tasks to a specialized subagent running in an isolated `pi` process, giving the subagent its own clean context window.
...
The returned text ends with the actual session ID:
